...
$sql = "SELECT * FROM applications WHERE id=? ORDER BY id DESC";
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            if ($row["group_head_status"] == 1) {
                ...
                if ($row["ad_status"] == 1) {
                    ...
                    if ($row["tcp_hr_head_status"] == 1) {
                        ...
                        if ($row["tcp_hr_ad_status"] == 1) {
                            ...
                            $status = 'Pending for TCP-HR AD Approval';
                        }
                        ...
                        $status = 'Pending for TCP-HR Head Approval';
                    }
                    ...
                    $status = 'Pending for AD Approval';
                }
                ...
                $status = 'Pending for Group Head Approval';
            }

            foreach ($progress_steps as $step => $percent) {
                $dot_class = 'progress-dot pending';

                if ($step === 'Application Submitted' || ($step === 'Accepted' && $current_progress === 100)) {
            ...
            if (!is_null($row['headquaters_remarks'])) {
                echo "<p class='remarks'><strong>Remarks:</strong> " . htmlspecialchars($row['headquaters_remarks']) . "</p>";
            } 
            if ($status === "Approved") {
